"""Accused ORM models matching the existing MySQL schema."""

import enum
from datetime import date, datetime
from sqlalchemy import DateTime, text

from sqlalchemy import (
    Boolean,
    Date,
    Enum,
    ForeignKey,
    Index,
    Integer,
    Numeric,
    String,
    TIMESTAMP,
    Text,
    text,
)
from sqlalchemy.orm import Mapped, mapped_column, relationship

from app.models.base import Base


class Gender(str, enum.Enum):
    MALE = "Male"
    FEMALE = "Female"
    OTHER = "Other"


class FIRAccusedLink(Base):
    """Junction table linking FIRs and accused persons."""

    __tablename__ = "fir_accused"

    fir_id: Mapped[int] = mapped_column(
        Integer,
        ForeignKey("firs.fir_id", ondelete="CASCADE"),
        primary_key=True,
    )

    accused_id: Mapped[int] = mapped_column(
        Integer,
        ForeignKey("accused.accused_id", ondelete="CASCADE"),
        primary_key=True,
    )

    fir = relationship("FIR")
    accused = relationship("Accused")

    def __repr__(self) -> str:
        return (
            f"<FIRAccusedLink("
            f"fir_id={self.fir_id}, "
            f"accused_id={self.accused_id})>"
        )


class Accused(Base):
    """Accused model matching the MySQL accused table."""

    __tablename__ = "accused"

    __table_args__ = (
        Index("idx_accused_name", "full_name"),
    )

    accused_id: Mapped[int] = mapped_column(
        Integer,
        primary_key=True,
        autoincrement=True,
    )

    full_name: Mapped[str] = mapped_column(
        String(100),
        nullable=False,
    )

    age: Mapped[int | None] = mapped_column(
        Integer,
        nullable=True,
    )

    gender: Mapped[Gender | None] = mapped_column(
        Enum(
            Gender,
            values_callable=lambda obj: [e.value for e in obj],
            native_enum=True,
        ),
        nullable=True,
    )

    dob: Mapped[date | None] = mapped_column(
        Date,
        nullable=True,
    )

    phone: Mapped[str | None] = mapped_column(
        String(20),
        nullable=True,
    )

    email: Mapped[str | None] = mapped_column(
        String(100),
        nullable=True,
    )

    address: Mapped[str | None] = mapped_column(
        Text,
        nullable=True,
    )

    occupation: Mapped[str | None] = mapped_column(
        String(100),
        nullable=True,
    )

    aadhaar_number: Mapped[str | None] = mapped_column(
        String(20),
        nullable=True,
    )

    risk_score: Mapped[float | None] = mapped_column(
        Numeric(5, 2),
        server_default=text("0.00"),
        nullable=True,
    )

    is_repeat_offender: Mapped[bool | None] = mapped_column(
        Boolean,
        server_default=text("0"),
        nullable=True,
    )

    created_at: Mapped[datetime | None] = mapped_column(
    DateTime,
    server_default=text("CURRENT_TIMESTAMP"),
    nullable=True,
)

    def __repr__(self) -> str:
        return (
            f"<Accused("
            f"accused_id={self.accused_id}, "
            f"full_name='{self.full_name}')>"
        )